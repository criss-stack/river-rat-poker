...
function wireEvents() {
  ...
  // Mobile-friendly toggles for settings and hand log
  const settingsToggle = document.getElementById('settings-toggle');
  const settingsDetails = document.getElementById('settings-panel');
  if (settingsToggle && settingsDetails) {
    settingsToggle.addEventListener('click', () => {
      const willOpen = !settingsDetails.open;
      settingsDetails.open = willOpen;
      settingsToggle.setAttribute('aria-expanded', String(willOpen));
    });
  }

  const logToggle = document.getElementById('log-toggle');
  const logDetails = document.getElementById('log-panel');
  if (logToggle && logDetails) {
    logToggle.addEventListener('click', () => {
      const willOpen = !logDetails.open;
      logDetails.open = willOpen;
      logToggle.setAttribute('aria-expanded', String(willOpen));
      // If opening log on small screens, ensure the log is scrolled into view
      if (willOpen) {
        const ol = document.getElementById('hand-log');
